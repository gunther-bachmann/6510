...
* functions (by group)
** pages
- [[file:vm-mm-pages.rkt::(define VM_INIT_MEMORY_MANAGER][VM_INIT_MEMORY_MANAGER]] ::    initialize memory management (must be called before first allocation)
  - racket include :: (require (only-in "./vm-mm-pages" VM_INIT_MEMORY_MANAGER))
