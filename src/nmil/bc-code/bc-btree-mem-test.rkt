...
#|
 ...
 tests to explicitily check for memory issues during btree operations
  ...
  (regression-test
   ...
   (check-equal? (shorten-cell-strings (vm-stack->strings b-tree-0-state 10 #t))
                  (list "stack holds 2 items"
                        "((1 . 2) . (3 . 4))  (rt)"
                        "NIL"))
   (check-equal? (length (vm-slots-free-in-page b-tree-0-state PAGE_AVAIL_0))
                  (- 42 3)
                  "after gc only 3 slots should be in use")
   (check-equal? (vm-num-slots-used-in-page b-tree-0-state PAGE_AVAIL_0)
                  3
...
(module+ test #| btree from-list, to-list |#
  ...
  (regression-test
   b-tree-1-state
   "list (1 2 3 4) -> btree -> list"
   (check-equal? (shorten-cell-strings (vm-stack->strings b-tree-1-state 10 #t))
                  (list "stack holds 2 items"
                        "(1 . (2 . (3 . (4 . NIL))))  (rt)"
                        "NIL"))
   (check-equal? (length (vm-slots-free-in-page b-tree-1-state PAGE_AVAIL_0))
                  (- 42 4)
                  "after gc 4 slots should remain in use")
   (check-equal? (vm-num-slots-used-in-page b-tree-1-state PAGE_AVAIL_0)
                  4
...
(module+ test #| btree from-list, path-to-first, add-value-after, to-list |#
  ...
  (regression-test
   b-tree-2-state
   "list (10 20 30 40 50 60) -> btree -> path to first -> add value 15 after -> root -> btree->list"
   (check-equal? (shorten-cell-strings (vm-stack->strings b-tree-2-state 10 #t))
                  (list "stack holds 2 items"
                        "(10 . (15 . (20 . (30 . (40 . (50 . (60 . NIL)))))))  (rt)"
                        "NIL"
                        ))
   (check-equal? (length (vm-slots-free-in-page b-tree-2-state PAGE_AVAIL_0))
                  38
                  "38 slots left on page 0") ;; 42  - 4
   (check-equal? (length (vm-slots-free-in-page b-tree-2-state PAGE_AVAIL_1))
                  39
                  "39 slots left on page 1") ;; 42  - 3
   (check-equal? (+ (vm-num-slots-used-in-page b-tree-2-state PAGE_AVAIL_0)
...
(module+ test #| btree from-list, path-to-first, add-value-after, to-list |#
  ...
  (regression-test
   b-tree-3-state
   "list (10 20 30 40 50 60) -> btree -> path-to-first -> add $15 after -> btree next -> add $25 after -> next -> next -> remove value -> root -> list "
   (check-equal? (shorten-cell-strings (vm-stack->strings b-tree-3-state 10 #t))
                 (list "stack holds 2 items"
                       "(10 . (15 . (20 . (25 . (30 . (50 . (60 . NIL)))))))  (rt)"
                       "NIL"
                       ))
   (check-equal? (vm-num-slots-used-in-page b-tree-3-state PAGE_AVAIL_0)
                 0
                 "page 0 is empty, no slots used anymore")
   (check-equal? (length (vm-slots-free-in-page b-tree-3-state PAGE_AVAIL_1))
                 35
                 "page 1 uses 7 slots and has 35 left")
   (check-equal? (vm-num-slots-used-in-page b-tree-3-state PAGE_AVAIL_1)
                 7
                 "is actually the number of cons cells (which is the number of dots in the list above)")
   (check-equal? (map (lambda (slot-offset)
  ...
  (regression-test
   ...
   (check-equal? (shorten-cell-strings (vm-stack->strings btree-reverse-0-state 10 #t))
                 (list "stack holds 2 items"
                       "(0 . 1)  (rt)"
                       "NIL"))
   (check-equal? (vm-num-slots-used-in-page btree-reverse-0-state PAGE_AVAIL_0)
                 1
                 "is actually the number of cons cells (which is the number of dots in the list above)")
   (check-equal? (map (lambda (slot-offset)
