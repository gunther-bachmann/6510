...
(provide vm-memory-manager
         ...
         VM_REFCOUNT_DECR_RT
          ...
          INIT_CELL_PAGE_A                            ;; allocate page and initialize for ref counted cells
          INIT_CELLPAIR_PAGE_A                       ;; allocate page and initialize for ref counted cell-pairs
...
(define VM_INITIALIZE_MEMORY_MANAGER
  ...
  ;;
  ;; input:  A = allocated uninitialized page
  ;; output: A = initialized page (of type cell page)
  ...
  (define INIT_CELL_PAGE_A
    (list
     (label INIT_CELL_PAGE_A)
            ;; page is in A
            (STA ZP_TEMP+1)
            (TAY)
            (LDA !$02)
...
(define VM_ALLOC_CELL_STACK_PAGES
  (list
   (label VM_ALLOC_CELL_STACK_PAGES)
         ...
         (JSR ALLOC_PAGE_TO_A)  ;; uses A and Y
         (STA ZP_TEMP+1)
         (LDA !$00)
         (STA ZP_TEMP)
         (LDY !$00)
...
(define INIT_CELLPAIR_PAGE_A
    (list
     (label INIT_CELLPAIR_PAGE_A)
            ;; page is in A
     ...
     (label SECOND_RC_BLOCK__INIT_CELLPAIR_PAGE_A)
            (STA (ZP_TEMP),y)
            (DEY)
            (BPL SECOND_RC_BLOCK__INIT_CELLPAIR_PAGE_A)
     ...
     (label SECOND_CELL_PAIR_BLOCK__INIT_CELLPAIR_PAGE_A)
            (TAY)
            (CLC)
            (ADC !$04)
            (STA (ZP_TEMP),y)
            (CMP !$F9)
            (BNE SECOND_CELL_PAIR_BLOCK__INIT_CELLPAIR_PAGE_A)
