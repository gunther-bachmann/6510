...
(define bc-opcode-definitions
  (list ;; opcode must start at 0 at increment by 2!
   (define-bc BC_PUSH_LOCAL_SHORT      PUSH_L0              #x00 1 "push l0") ;; ensure opcode & #x06 = 0
   (define-bc BC_PUSH_LOCAL_SHORT      PUSH_L1              #x02 1 "push l1") ;; ensure opcode & #x06 = 2
   (define-bc BC_PUSH_LOCAL_SHORT      PUSH_L2              #x04 1 "push l2") ;; ensure opcode & #x06 = 4
   (define-bc BC_PUSH_LOCAL_SHORT      PUSH_L3              #x06 1 "push l3") ;; ensure opcode & #x06 = 6
   (def-xt-bc BC_EXT1_CMD                                   #x08
    (list (define-bc VM_INTERPRETER_INC_PC_2_TIMES _             #x00 1 "reserved")  ;; opcode must start at 0 and increment by 1
          (define-bc BC_IMAX           IMAX                      #x01 1 "int max")
          (define-bc BC_IINC           IINC                      #x02 1 "int inc")
          (define-bc BC_GC_FL          GC                        #x03 1 "gc")))

   (define-bc VM_INTERPRETER_INC_PC    _                    #x0a 1 "reserved") ;; reserved
   (define-bc BC_PUSH_I                PUSH_I               #x0c 3
                 (lambda (_l _bc bc-p1 bc-p2) (format "push int $~a" (format-hex-word (bytes->int bc-p1 bc-p2)))))
   (define-bc BC_INT_P                 INT_P                #x0e 1 "int?")
   (define-bc BC_WRITE_LOCAL_SHORT     WRITE_L0             #x10 1 "write l0") ;; ensure opcode & #x06 = 0
   (define-bc BC_WRITE_LOCAL_SHORT     WRITE_L1             #x12 1 "write l1") ;; ensure opcode & #x06 = 2
   (define-bc BC_WRITE_LOCAL_SHORT     WRITE_L2             #x14 1 "write l2") ;; ensure opcode & #x06 = 4
   (define-bc BC_WRITE_LOCAL_SHORT     WRITE_L3             #x16 1 "write l3") ;; ensure opcode & #x06 = 6
   (define-bc BC_T_P_BRA               T_P_BRA              #x18 2
                 (lambda (_l _bc bc-p1 _bc-p2) (format "branch on true? by $~a" (format-hex-byte bc-p1))))
   (define-bc BC_F_P_BRA               F_P_BRA              #x1a 2
                 (lambda (_l _bc bc-p1 _bc-p2) (format "branch on false? by $~a" (format-hex-byte bc-p1))))
   (define-bc BC_F_P_RET               F_P_RET              #x1c 1 "ret on false?")
   (define-bc BC_DUP                   DUP                  #x1e 1 "int?")
   (define-bc BC_POP_TO_LOCAL_SHORT    POP_TO_L0            #x20 1 "pop to l0") ;; ensure opcode & #x06 = 0
   (define-bc BC_POP_TO_LOCAL_SHORT    POP_TO_L1            #x22 1 "pop to l1") ;; ensure opcode & #x06 = 2
   (define-bc BC_POP_TO_LOCAL_SHORT    POP_TO_L2            #x24 1 "pop to l2") ;; ensure opcode & #x06 = 4
   (define-bc BC_POP_TO_LOCAL_SHORT    POP_TO_L3            #x26 1 "pop to l3") ;; ensure opcode & #x06 = 6
   (define-bc BC_PUSH_NIL              PUSH_NIL             #x28 1 "push nil")
   (define-bc BC_PUSH_AF               PUSH_AF              #x2a 1 "push array field")
   (define-bc BC_POP_TO_AF             POP_TO_AF            #x2c 1 "pop to array field")
   (define-bc BC_PUSH_B                PUSH_B               #x2e 2
                 (lambda (_l _bc bc-p1 _bc_p2) (format "push byte $~a" (format-hex-byte bc-p1))))
   (define-bc BC_WRITE_TO_LOCAL_SHORT  WRITE_TO_L0          #x30 1 "write to l0") ;; ensure opcode & #x06 = 0
   (define-bc BC_WRITE_TO_LOCAL_SHORT  WRITE_TO_L1          #x32 1 "write to l1") ;; ensure opcode & #x06 = 2
   (define-bc BC_WRITE_TO_LOCAL_SHORT  WRITE_TO_L2          #x34 1 "write to l2") ;; ensure opcode & #x06 = 4
   (define-bc BC_WRITE_TO_LOCAL_SHORT  WRITE_TO_L3          #x36 1 "write to l3") ;; ensure opcode & #x06 = 6
   (define-bc BC_BINC                  BINC                 #x38 1 "byte inc")
   (define-bc BC_NZ_P_BRA              NZ_P_BRA             #x3a 2
                 (lambda (_l _bc bc-p1 _bc-p2) (format "branch on not zero? by $~a" (format-hex-byte bc-p1))))
   (define-bc BC_CELL_EQ_P             CELL_EQ_P            #x3c 1 "cell eq?")
   (define-bc BC_F_P_RET_F             F_P_RET_F            #x3e 1 "ret false on false?")
   (define-bc VM_INTERPRETER_INC_PC    _                    #x40 1 "reserved") ;; reserved
   (define-bc BC_NIL_P                 NIL_P                #x42 1 "nil?")
   (define-bc BC_I_Z_P                 I_Z_P                #x44 1 "int 0?")
   (define-bc BC_BADD                  BADD                 #x46 1 "byte +")
   (define-bc BC_B_GT_P                B_GT_P               #x48 1 "byte >?")
   (define-bc BC_NATIVE                NATIVE               #x4a 1 "native") ;; need to be fixed on 4a, implementation dependes on it
   (define-bc BC_B_GE_P                B_GE_P               #x4c 1 "byte >=?")
   (define-bc BC_BSHR                  BSHR                 #x4e 1 "byte shift right")
   (define-bc VM_INTERPRETER_INC_PC    _                    #x50 1 "reserved") ;; reserved
   (define-bc BC_CxxR                  CDDR                 #x52 1 "cddr") ;; CDDR (bitwise-and #x1f x) must be #x12 (18)!
   (define-bc BC_BREAK                 BREAK                #x54 1 "break")
   (define-bc BC_SWAP                  SWAP                 #x56 1 "swap")
   (define-bc BC_POP                   POP                  #x58 1 "pop")
   (define-bc BC_CONS_PAIR_P           CONS_PAIR_P          #x5a 1 "pair?")
   (define-bc BC_T_P_RET               T_P_RET              #x5c 1 "ret on true?")
   (define-bc BC_Z_P_BRA               Z_P_BRA              #x5e 2
   ...
   (define-bc BC_BDEC                  BDEC                #x6c 1 "byte dec")
   (define-bc BC_CONS                  CONS                #x6e 1 "cons")
   (define-bc BC_PUSH_INT0             PUSH_I0             #x70 1 "push int 0") ;; ensure opcode & #x06 = 0
   (define-bc BC_PUSH_INT1             PUSH_I1             #x72 1 "push int 1") ;; ensure opcode & #x06 = 2
   (define-bc BC_PUSH_INT2             PUSH_I2             #x74 1 "push int 2") ;; ensure opcode & #x06 = 4
   (define-bc BC_PUSH_INTm1            PUSH_IM1            #x76 1 "push int -1") ;; ensure opcode & #x06 = 6
   (define-bc BC_GOTO                  GOTO                #x78 2
   ...
   (define-bc BC_CDR                   CDR                 #x7e 1 "cdr")
   (define-bc BC_Z_P_RET_POP_N         Z_P_RET_POP_1       #x80 1 "ret on zero? and pop 1") ;; ensure opcode & #x06 = 0
   (define-bc BC_Z_P_RET_POP_N         Z_P_RET_POP_2       #x82 1 "ret on zero? and pop 2") ;; ensure opcode & #x06 = 2
   (define-bc BC_Z_P_RET_POP_N         Z_P_RET_POP_3       #x84 1 "ret on zero? and pop 3") ;; ensure opcode & #x06 = 4
   (define-bc BC_Z_P_RET_POP_N         Z_P_RET_POP_4       #x86 1 "ret on zero? and pop 4") ;; ensure opcode & #x06 = 6
   (define-bc BC_COONS                 COONS               #x88 1 "coons")
   (define-bc BC_SWAP_RA_RB            SWAP_RA_RB          #x8a 1 "swap RA<->RB")
   (define-bc BC_POP_TO_RB             POP_TO_RB           #x8c 1 "pop to rb")
   (define-bc BC_PUSH_RA               PUSH_RA             #x8e 1 "push ra")
   ...
   (define-bc BC_ALLOC_ARA             ALLOC_ARA           #x98 1 "alloc array to ra")
   (define-bc BC_PUSH_RA_AF            PUSH_RA_AF          #x9a 1 "push (ra),rai")
   (define-bc BC_POP_TO_RA_AF          POP_TO_RA_AF        #x9c 1 "pop to (ra),rai")
   (define-bc BC_POP_TO_RAI            POP_TO_RAI          #x9e 1 "pop byte to rai")
   (define-bc BC_PUSH_LX_CAR           PUSH_L0_CAR         #xa0 1 "push (car l0)") ;; ensure opcode & #x06 = 0
   (define-bc BC_PUSH_LX_CAR           PUSH_L1_CAR         #xa2 1 "push (car l1)") ;; ensure opcode & #x06 = 2
   (define-bc BC_PUSH_LX_CAR           PUSH_L2_CAR         #xa4 1 "push (car l2)") ;; ensure opcode & #x06 = 4
   (define-bc BC_PUSH_LX_CAR           PUSH_L3_CAR         #xa6 1 "push (car l3)") ;; ensure opcode & #x06 = 6
   ...
   (define-bc BC_WRITE_RA              WRITE_RA            #xaa 1 "write ra")
   (define-bc BC_WRITE_TO_RAI          WRITE_TO_RAI        #xac 1 "write byte to rai")
   ...
   (define-bc BC_CAR                   CAR                 #xba 1 "car")
   (define-bc BC_ISUB                  ISUB                #xbc 1 "int -")
   (define-bc BC_IADD                  IADD                #xbe 1 "int +")
