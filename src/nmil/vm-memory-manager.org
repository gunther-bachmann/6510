...
* open tasks
** TODO replace NEW_ with regular naming              -> clean code
** TODO rename free-lists (use more expressive names) ->
** TODO provide labels for byte code debugger         -> increase debugger usability (allow label usage in byte-code debugger and during dive!)
** TODO provide labels for interactive 6510 debugger  -> increase debugger usability (allow label usage in byte-code debugger and during dive!)
...
** TODO idea for runtime profiling                    -> evaluate optimizations
...
|--------------------------------------------------+-----------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| NEW_DEC_REFCNT_CELL_RC                           | i               | specific dec refcnt cell, possible free if refcnt drops to 0                                                                                                                 |
...
| NEW_DEC_REFCNT_CELLPAIR_RC                       | i               | specific dec refcount of cellpair, possible free if refcnt drops to 0                                                                                                        |
|--------------------------------------------------+-----------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| NEW_DEC_REFCNT_M1_SLOT_RC                        | i               | specific dec refcnt of m1 slot, possible free if refcnt drops to 0                                                                                                           |
| NEW_DEC_REFCNT_CELLARR_RC                        | i               | specific dec refcnt of cellar, possible free if refcnt drops to 0                                                                                                            |
| NEW_DEC_REFCNT_NATIVEARR_RC                      | i               | specific dec refcnt of nativearr, possible free if refcnt drops to 0                                                                                                         |
| NEW_DEC_REFCNT_RC                                | t               | generic dec refcnt rc, calls more specific routines once the ptr type was detected                                                                                           |
|--------------------------------------------------+-----------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| FREE_M1_SLOT_RCm1                                | i               | generic free m1 slot (currently pointing to the ref count field)                                                                                                             |
|--------------------------------------------------+-----------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| NEW_FREE_M1_SLOT_RC                              | i               | generic free for m1 slots, calls more specific routines once slot type was detected (currently either cellarr or nativearr)                                                  |
|--------------------------------------------------+-----------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
| NEW_FREE_CELLPAIR_RC                             | t               | specific free for cellpair ptr                                                                                                                                               |
|--------------------------------------------------+-----------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| NEW_FREE_CELLARR_RC                              | -               | specific free cellarr, free non ptr cells in array (starting at end), decref ptrs until first ptr is encountered that must be freed                                          |
| NEW_FREE_NATIVEARR_RC                            | /               | simply use NEW_ADD_M1_SLOT_RC_TO_PFL (no further actions necessary for deallocation)                                                                                         |
|--------------------------------------------------+-----------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| NEW_FREE_CELL_RT                                 | t               | prefix new_free_cell_rc with copy rt->rc                                                                                                                                     |
| NEW_FREE_CELL_RA                                 | t               | prefix new_free_cell_rc with copy ra->rc                                                                                                                                     |
| NEW_FREE_CELL_RC                                 | t               | generic, call specific routines once cell type is detected                                                                                                                   |
|--------------------------------------------------+-----------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
| NEW_INC_REFCNT_RC                                | planned         | generic, call specific routines once cell type is detected                                                                                                                   |
|--------------------------------------------------+-----------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| NEW_INC_REFCNT_CELL_RC                           | planned         | specific increment refcount of cell ptr                                                                                                                                      |
| NEW_INC_REFCNT_CELLPAIR_RC                       | planned         | specific increment refcount of cellpair ptr                                                                                                                                  |
| NEW_INC_REFCNT_CELLARR_RC                        | planned         | specific increment refcount of cell array ptr                                                                                                                                |
| NEW_INC_REFCNT_NATIVEARR_RC                      | planned         | specific increment refcount of native array ptr                                                                                                                              |
| NEW_INC_REFCNT_M1_SLOT_RC                        | planned         | generic increment refcount of m1 slot (no need for calling more specific routines)                                                                                           |
|--------------------------------------------------+-----------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| NEW_ADD_M1_SLOT_RC_TO_PFL                        | i               | put the m1 slot pointed to by rc into the page free list (and do some housekeeping, concerning the list of pages with free slots)                                            |
| NEW_DROP_FULL_PAGES_AT_HEAD_OF_M1_PAGE_RC        | i               | put page used in rc+1 to a, then NEW_DROP_FULL_PAGES_AT_HEAD_OF_M1_PAGE_A                                                                                                    |
...
| NEW_PUT_PAGE_AS_HEAD_OF_M1_PAGE_RC               | i               | put the page in rc (which must have free slots) as new head of VM_FREE_M1_PAGE_Px                                                                                            |
|--------------------------------------------------+-----------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| GC_INCR_CELLPAIR_GFL                             | planned         | incremental gc cellpairs in global free list                                                                                                                                 |
| GC_INCR_CELLARR_GFL                              | i               | incremental gc cell arrays in global free list (does prefix copy gfl of cell-arrays into rc, then continue with NEW_GC_INCR_ARRAY_SLOT_RC, immediate return if gfl is empty) |
| NEW_GC_INCR_ARRAY_SLOT_RC                        | i               | incremental dec refcnt cells in this cell array (max 1 in this call) and adjust the number of elements to the # not checked yet, add slot tp pfl if 0 left                   |
