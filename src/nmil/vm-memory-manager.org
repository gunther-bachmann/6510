...
functions called BC_... must be refcnt'd correctly,
functions called VM_... may be refcnt'd correctly (adding REFCNTD postfix)
other functions do no refcnt (except for very special cases <- check which are necessary!)
...
** TODO implement RTM (or maybe RP) as TOS-1 register on demand
