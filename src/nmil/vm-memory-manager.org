...
- example: replace all labels and jumps to labels that end with ~__~ with ~__some_funcion~
 ...
 #+end_src
** TODO idea
instead of implementing free for RT and RA, implement one function, copying RT/RA to (e.g. RC) and work on RC only!
...
- translation table
  ...
  | x    | VM_CELL_STACK_PUSH_R                        | PUSH_CELL_RT_TO_EVLSTK             |      |                                                                    |
  | x    | ;; vm_cell_stack_push_rt_if_nonempty        | PUSH_CELL_RT_TO_EVLSTK_IF_NONEMPTY |      |                                                                    |
