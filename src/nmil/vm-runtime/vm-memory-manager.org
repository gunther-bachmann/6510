...
check call frame page has sufficient space for call (# locals + 2)
  does not fit:
    allocate 2 new call frame pages (and init)
    set respective previous pages to new call frame pages
...
*** pointers
tag byte ends on bits 0 [m1-ptr], or 01 [cell-pair-ptr]
points somewhere.
- it points to a complex cell (on a m1 page), e.g. cell-array, native array
...
- bcd, symbol, ...
*** header cell
is a cell that is used as a header for some payload. the following headers exist
0lll llll native array (length 1..82)
10ll llll cell-array   (length 1..41)
110x xxxx function     (0..7 constants, static/dynamic)
1110 ffff float
1111 xxxx unused
- header fof a *native array of bytes* (useful for strings)
  tag byte = 0lll llll (and $7f = len)
  offset  content        <- memory layout
  ------------------
  00      header-byte including length of byte array
  01      byte index 0
  ...
  n+1     byte index n
- header for an *array of cells* (useful for structures)
  ...
  offset  content        <- memory layout
  ------------------
  00      header-byte including length of cell-array
  01..02  cell index 0 *)
  03..04  cell index 1
  ...
  ..n*2+2 cell index n
...
- header for *(dynamic) functions*
  ...
  01..02  constant 0
   ..n*2  constant n
  n*2+1   function header byte    <-- func-ptr
  n*2+2   bc0
          ...
...
- header for a *float*
  ...
  offset  content        <- memory layout
  ------------------
  -1      reference count of this field (including top 4 bits of 5th byte of float)
  00      header-byte (including 4 bits of 5th byte of float)
  01.04   4 bytes of the float
...
- idea: what if pages have different bucket sizes?
  ...
  - does not help with pointer tags using the last 2 bits
  - possible sizes must be multiples of 4
    8 (* 31 8) => 248
