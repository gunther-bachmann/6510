...
- [X] do some scroll / text output benchmarks
  - [X] fill screen with 0..9, and alternating color, then scroll (and record timing), then clear screen, then output time spent
    - left 40 times
    - right 40 times
    - down 24 times
    - up 24 times
  - [X] run benchmark on c64
- [ ] implement loading of constants (saved in a serialized way)
