...
- byte code (no gc necessary)
  ...
  #+begin_src scheme
    (byte-cell 10 l0) ;; create a byte cell with value 10 at local 0
    (byte-cell 20 l1)
    (make-list 2 l0 l1 l2) ;; make  a list of l0 and l1 into l2/l3 [optimization, could reuse l0 and l1 for list]
    (byte-cell 0 l4) ;; make default argument explicitly at caller site
    (call 'sum 3 l2 l3 l4 -0-) ;; call sum with two input arguments, l2, l3, storing result in l4 (=> 3 parameters), allocating additional 0 locals
    ...
    (byte+ a2 a0 a2)
    (resolve-ptr-arg-into-arg a1 2 a0) ;; resolve ptr in cdr cell (arg 1), 2 consequtive cells (another list, that is car and cdr-cell) into arg 0 (and 1)
    (goto -sum-recur) ;; tailcall
    (label -sum2)
    (return) ;; a2 (last parameter) is automatically the return value
  #+end_src
...
- data layout
  byte offset
  0 [data-header-cell = struct header] => some bytes are following
  2 [struct-meta-data (id)]
  4 [struct-data-bytes]
- bytecode
  #+begin_src scheme
...
- define interpreter (poc)
  ...
  #+begin_src scheme
    ...
    (def-struct call-frame
      ...
      (lnps         : (array cell))  "locals and parameters array (len defined by function) + actually passed parameter")
    ...
    (def (vm-new) : vm
         ...
         (vm l() [(function [0] "just-break" 0)] (call-frame (continuation 0 0) 0 0 [] [])))
    ...
    (def (bytes->int (low : byte) (high: byte)): int
         (def-local int-cell (make-int-cell))
         (int-cell-low-set! int-cell low)
         (int-cell-high-set! int-cell high)
         int-cell)
    ...
    (def (vm-decode-operand-ptr (op-code : byte) (avm : vm)) : cell
         ...
         (if (bitwise-and op-code 0x00001000) ;; is access to local | arg, else access to global
             (nth (call-frame-lnps (vm-call-frame avm)) (bitwise-and op-code 0x00000111))
...
- collected-commands
  ...
  - byte+ :: add top 2 byte cells, leaving one sum byte cell an the eval stack
  - call <fun> <#params> <p0> ... <pN> :: call a specific function (by ptr/int-id) + parameter (#locals is got from the function list/array)
  ...
  - make-list <num> :: create a list of the <num> topmost cells on the stack, putting car and cdr on the stack
  - pop :: discard tos
  - pop-into-arg <reg> :: pop tos into the given arg
  - push-byte <val> :: push a byte value into a cell on the eval stack
  - return :: simple return
  - struct-create <label> :: allocate structure bytes, set cell-header, metadata and data, put ptr to struct onto stack?
  - struct-get <data-off> :: get cell at data-off to tos (struct ptr in tos-2)
  - struct-set <data-off> :: set cell at data-off to tos (struct ptr in tos-2)
** memory management
*** layout
**** operations
...
**** collected code
- cisc-code for builtin functions (not runtime) => no eval stack, all is directly referencing args, locals or globals
  - byte/int+/- a/l/g# a/l/g# -> a/l/g#
  - byte/int-inc/dec
  - byte/int*//
  - zerob?
  - zeroi?
  ...
  - bra-greaterb/i?
  - bra-equalb/i?
  - bra-greater-or-equalb/i?
  - goto/jump
  - bra-nil?
  - land/or/not
  - cons a/l/g# a/l/g# -> a/l/g#
  - car/cons = index0 index1 of list double cell
  - struct-set a/l/g# a/l/g# a/l/g# :: set structure arg0, field# in arg1, to value arg2
  - struct-get a/l/g# a/l/g# a/l/g# :: get structure arg0, field# in arg1 into arg2
*** data types
...
- cell
  ...
  - header (need max bits, describing the type of the header + as much information as possible)
    ...
    layout: [type ()] .....010
  - int (need max bits, low bits should all be zero to allow adding/substraction without shifting)
    layout: [value (x bits)] ......00
    range: 0..16383, -8192..8191
  - byte/char (need 8 bits, one full byte should be used unmodified)
    layout: [value (8 bits)] .....110
