...
- provide function w/ ref and w/o ref counting <- for optimization
  - name-convention: function w/ ref counting: ...​_RC
...
**** reference count (only pointers to cells and pointers to cell-pais)
...
- TODO
   ...
   first complete all commands to increment the ref count, once this is complete, the decrement can be implemented (and checked)
   ...
   GOTO :: do nothing
   POP :: if rt is ptr, dec ref
   POP​_TO​_LOCAL​_x :: if local_x is a ptr, dec ref of local x (before overwriting it)
   CONS
   - check top two elements on the stack and inc ref if ptr
   - set ref count to 1 on new allocated cell
   COONS
   - check top three elements on stack and inc ref if ptr
