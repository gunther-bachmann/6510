...
- [X] implement basic byte code debugger on top of 6510 debugger
  ...
  - [X] step over call
**** add profiler for code length
**** enable restting cpu cycle count (e.g. via extended byte codes)
**** optimize byte code
- push_​local_​0, write_​to_​local_​0 :: make this a one byte command (not a short instruction) => speeds up decoding
- cadr cdar caar cddr :: could be put into a fast instruction
...
**** enhance debugger
- [ ] pretty print the next x byte code instructions
