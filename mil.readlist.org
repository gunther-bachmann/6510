...
**** DONE write a 6510 vm interpreter (partially, validating some design decisions)
- alternative: use 128 (non short) commands with bit0 always 0 => LDA ($PC),Y -> STA JMPOP+1 -> JMP OPTBL (8 bytes, 12-cycles)
...
**** allow 6510 interpreter to validate designed vm assembly code
- write assembly code, validate behavior with test code <- make this easy/feasable
**** write executable 6510 vm for (some) commands
...
**** implement (partial) vm in 6510 assembler => validation of vm implementation feasability (byte code size + interpretation speed)
**** implement vm in mil
