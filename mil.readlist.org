...
** TODO intermediate code (1984)
https://gtoal.com/imp77/reference-manual/icode.html
- Absolute      tos := | tos |
- Access        A_sos[tos] := Value (@PC?)
- Add           tos := tos + sos
- Address       tos := PTR to tos-referenced object
  replace tos with address of object
- Adjust        sos += tos * item size of sos
- Alias         ?
- Alt-Start     mark start of alternative def of tags
- Alt-Finish    mark end  of alternative def of tags
- And           tos := sos & tos (binary and)
- Assign-Parameter tos is passed as parameter to sos
- Assign-Value  tos is assigned to item referred to by sos
- Assign-Reference sos is made to point to var referenced by tos
- Backward <label> jump unconditionally to label
- Begin         begin of anonymous  function
- BEQ <label>   branch on sos == tos
- BF <label>    branch on tos == false
- BGE <label>   branch on tos >= sos
- BGT <label>   branch on tos > sos
- BLE
- BLT
- BNE
- Bounds        tos = upper bound, sos = lower bound
- BT            branch on tos == true
- Byte <b>      value <b> is stacked
- Call          procedure call
- Compare-References addr(tos) == addr(sos)
- Compare-Repeated-Values
- Compare-Unsigned-Values
- Compare-Values
- Complement    tos = one complement of tos
- Concat        conccat tos and sos
- Control <n>
- Define <tag> [id] <a> <b> <c> create new tag value
- Define-Range <tag>
- Diagnose <n>
- Dimension <n><d>
- Div
...
- Variable-Call
- Xor
** IMPLEMENT 6502 mint :vm:sc:mem:
https://github.com/orgMINT/MINT
https://github.com/agsb/6502.MINT/blob/main/arch/6502/6502.MINT.s
https://github.com/monsonite/MINT
** DONE tiny basic byte code
...
[[file:~/dl-scanned/compiler/vdoc.pub_compiler-design-virtual-machines.pdf][book]], 193p, 2010
- *no* cookbook!
...
**** DONE implement some code for arrays (structures)
- State "DONE"       from "IMPLEMENT"  [2025-06-16 Mon 22:28]
...
**** DONE bc optimization - looping with index registers
- [X] combine POP n, PUSH_​RA -> WRITE_​RA
- [X] looping with RAI, RBI as index -> DEC_​RAI_​NZ_​P_​BRA (decrement RAI, if nz branch)
- (combine POP n, PUSH_​RA, RET -> RET_​RA_​POP n)
- [X] combine DUP, POP_​TO​_​RAI -> WRITE_​TO_​RAI
- (combine POP, PUSH_​B b -> WRITE_​B b)
...
**** IDEA page defragmentation
***** age / longivity of object
- is higher if on the stack when leaving a function
- is prop. to the stack eval stack depth
***** need to defrag arises, if sum of free slots within all pages of one slot size > a page
***** defrag could be initiated, iff:
- memory is running low (# pages free < # pages used)
- free slots in sum > 1 page (for one slot size), using 4 pages max
=> moving these slots into less pages will free 1 page
   select objects to be moved (from page with the lowest number of used slots)

- find all references to these objects
  use refcount to know when all references were found
- scan eval stack + call-frames (w/ parameters) to search for matching references
- follow stack + call-frame references into the heap to check for references
- make sure to quickly identify refs to this page (share same page portion of the ptr)
- find algorithm to have only tailcall recursion? (no additional heap nor stack usage!)
***** use rc byte for more information
[ma--cccc]
cccc 4 bit for actual # of references
m currently marked
a age (old)
- reserved
***** during defrag, make temp usage of free slots on page to be freed (e.g. min 30% free)
...
*** deallocation
** continuations
** (outdated) vm registers and runtime behavior
