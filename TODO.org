...
e.g. INC​_REFCNT​_CEL​_RT

- nouns
  CEL :: cell
  CELy :: cell at y of either cell-array or cell-pair (depending on context), y = 0, 2, 4 ... <- actual offset
  CELPAR :: cell-pair
  CELARR :: cell-array (is a vslot)
  M1Px :: m1 page of profile x (can be register x)
  PAGE :: page
  REFCNT :: refcount
  EVLSTK :: cell eval stack
  RT :: top of cell eval stack
  FRMSTK :: frame stack
  TOSmr :: top of stack - r (can be a constant or a register)
  SLOT :: allocation unit within a page (CEL, CELPAR, VSLOT is a SLOT)
