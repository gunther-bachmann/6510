...
(define addressing-program
  (list
   ...
   (LDA < "$4000",x >)    ;; indirect x (add x to $4000, read pointer (low+high) from memory, read pointed to byte)
   (LDA < "$4000" >,y)    ;; indirect y (read pointer from $4000, add y to pointer, read pointed to byte)
   (LABEL ":NEXT")
